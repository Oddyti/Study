...
class ChessAI():
	...
	def __init__(self, chess_len):
		...
		self.human, self.pc = 1, 2
		self.win = 0

	# 将pc_count,human_count,pc_score,human_score归零
	def reset(self):
	...
	# 统计一条线上的双方棋型
	def count_line_type(self, str_line):	
	...
	# 如果pc或者human下出这个空点的整体棋局分数,返回pc_score和human_score
	def calc_if(self, board, x, y, who):
	...
	# pc根据全局的count和score生成可下棋点有序列表moves = (score, x, y)
		...
		for x in range(self.len):
			for y in range(self.len):				
				if board[x][y] == 0 and self.has_neighbor(board, x, y, radius):		
					pc_score,human_score = self.calc_if(board, x, y, self.pc)
					point = (pc_score - human_score, x, y)
					moves.append(point)
	...
	def max_min(self, board, moves):
		judge = []
		
		for one_move in moves:
			move_x = one_move[1]
			move_y = one_move[2]
			score = self.max_search(board, move_x, move_y, moves, depth = AI_SEARCH_DEPTH)
			point = (score, move_x, move_y)
			judge.append(point)
		
		judge.sort(reverse = True)
		best_score, best_x, best_y = judge[0]
		return best_score, best_x, best_y

	def max_search(self, board, max_x, max_y, moves, depth, alpha = SCORE_MIN, beta = SCORE_MAX):
		if board[max_x][max_y] == 0:
			board[max_x][max_y] = 2

			for every_move in moves:
				move_x, move_y = every_move[1], every_move[2]
				if(board[move_x][move_y] == 0):
					child_node_value = self.min_search(board, move_x, move_y, moves, depth - 1, alpha, beta)
					
					if child_node_value > alpha:
						alpha = child_node_value
					if child_node_value >= beta:
						break
			
			board[max_x][max_y] = 0

		if(depth == 0):
			temp_pc_score, temp_human_score = self.calc_if(board, max_x, max_y, self.pc)
			global_score = temp_pc_score - temp_human_score
			alpha = global_score
				
		return alpha
	
	def min_search(self, board, min_x, min_y, moves, depth, alpha = SCORE_MIN, beta = SCORE_MAX):
		if board[min_x][min_y] == 0:
			board[min_x][min_y] = 1	

			for every_move in moves:
				move_x, move_y = every_move[1],every_move[2]
				if(board[move_x][move_y] == 0):
					child_node_value = self.max_search(board, move_x, move_y, moves,  depth - 1, alpha, beta,)
		...
		if(depth == 0):
			temp_pc_score, temp_human_score = self.calc_if(board, min_x, min_y, self.human)
			global_score = temp_pc_score - temp_human_score
			beta = global_score
	...
	def find_bestmove(self, board):
		...
		best_score, best_x, best_y = self.max_min(board, moves)
