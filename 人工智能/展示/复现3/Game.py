...
class Game():
    def __init__(self):
        ...
        self.buttons.append(buttonAgain(self.screen, '再来', 20 + BUTTONWIDTH*2 + 20*2, (NAVBARHEIGHT-BUTTONHEIGHT)/2))
    ...
    def startGame(self):
        ...
        self.player = 1
        self.winner = 0
        self.board.boardReset()  # 重置棋盘
        # print(self.board.chessMap)
        ...
        if self.isplaying and not self.isover:
            if self.winner  == 0:
                if self.useAI:
                    ...
                    # self.AI.is_win()
                    # self.winner = self.AI.win
                    self.checkClick(x, y, True)
                    self.AI.find_bestmove(self.board.chessMap)
                    self.AI.is_win()
                    for i in self.board.chessMap:
                        print(i)
                    print('Use Ai, winner: %d, AI.win: %d' % (self.winner, self.AI.win))
                    self.useAI = False
                    ...
                    for i in self.board.chessMap:
                        print(i)
                    print('Not Use Ai, winner: %d, AI.win: %d' % (self.winner, self.AI.win))
            else:
                self.showMessage(5)
                # print('获胜了 winner is %d' % self.winner)
                # print(self.board.chessMap)
                self.AI.win = 0
                self.isover = True
...
class buttonStart(Buttons):
    ...
    def click(self, game):
        if self.enable:
            game.startGame()
            print('CLICK START: isplaying %d,  winner %d,  useAI %d, isover %d' % (game.isplaying, game.winner , game.useAI, game.isover))
            self.messageText = self.font.render(self.text, True, 'white', BUTTONCOLOR[1])
            self.enable = False
            return True
        else:
...
class buttonGiveUp(Buttons):
    ...
    def click(self, game):
        if self.enable:
            ...
            print('CLICK GIVEUP: isplaying %d,  winner %d,  useAI %d, isover %d' % (game.isplaying, game.winner , game.useAI, game.isover))
            self.messageText = self.font.render(self.text, True, 'white', BUTTONCOLOR[1])
            self.enable = False
            return True
        return False
...
class buttonAgain(Buttons):
    ...
    def click(self, game):
        if self.enable:
            game.isover = False
            print('CLICK AGAIN: isplaying %d,  winner %d,  useAI %d, isover %d' % (game.isplaying, game.winner , game.useAI, game.isover))
            self.messageText = self.font.render(self.text, True, 'white', BUTTONCOLOR[1])
            self.enable = False
            return True
        else:
