...
figure(1);
subplot(3,2,1);plot(w,y);title('原信号冲激采样频谱');axis([-3 3 -0.1 1.2]);grid on;
subplot(3,2,2);plot(w,H1w);title('理想的矫正滤波器H(jw)');axis([-8 8 1 1.6]);grid on;
subplot(3,2,3);plot(w,H1w_new);title('重建Hr''(jw)');axis([-8 8 1 1.6]);grid on;
subplot(3,2,4);plot(w,Hjw);title('重建后的联合滤波');axis([-100 100 -0.3 0.6]);grid on;
subplot(3,2,5);plot(w,x1w);title('未矫正频域');axis([-3 3 -0.1 1.2]);grid on;
subplot(3,2,6);plot(w,x2w);title('重建矫正后频域');axis([-3 3 -0.1 1.2]);grid on;
