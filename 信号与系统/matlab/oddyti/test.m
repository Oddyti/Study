...
figure(1);
subplot(3,2,1);plot(w,y);title('输入信号');axis([-3 3 -0.1 1.2]);grid on;
subplot(3,2,2);plot(w,H1w);title('理想的矫正滤波器H(jw)');axis([-8 8 1 1.6]);grid on;
subplot(3,2,3);plot(w,H1w_new);title('重建后的Hr''(jw)');axis([-8 8 1 1.6]);grid on;
subplot(3,2,4);plot(w,Hjw);title('重建后的联合滤波');axis([-100 100 -0.3 0.6]);grid on;
subplot(3,2,5);plot(w,x1w);title('未矫正重建输入信号');axis([-3 3 -0.1 1.2]);grid on;
subplot(3,2,6);plot(w,x2w);title('矫正后重建输入信号');axis([-3 3 -0.1 1.2]);grid on;
