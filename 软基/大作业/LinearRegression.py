...
# 构建训练集和测试集
#  前80%划分为训练集，后20%为测试集
X_train, X_test, Y_train, Y_test = train_test_split(df.iloc[:, :9], df.y, train_size=.80, random_state=1)
...
#显示图像
# plt.show()
print("mse", np.sqrt(MSE(Y_test,Y_pred))/Y_test.mean())
# 预测值与测试值对比
plt.figure()
plt.plot(range(len(Y_pred)), Y_pred, 'b', label="predict")
...
plt.savefig('Linear', dpi = 300, figsize=[10,10])
plt.show()
