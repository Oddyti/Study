...
上一节讲到，当$T> \displaystyle \frac{1}{2W}$，即$\displaystyle \frac{1}{T} < 2W$时，可以通过设计信号波形使得满足$X\left(f+\displaystyle \frac{m}{T}\right)=T$条件，从而达到无码间干扰。而升余弦频谱信号是最常用的一种无码间干扰波形，其频率响应、时域波形如下：
$$
X_{r c}(f)=\left\{
\begin{array}{cc}
...
\end{array}\right.
\\
x(t)=sinc(t/T)\cdot \frac{\cos(\pi\alpha t/T)}{1-4\alpha^2t^2/T^2}
$$

其中$\alpha$称为滚降因子，$0 \leq \alpha \leq 1$

<img src="升余弦.png" alt="升余弦" style="zoom:50%;" />

#### 六、具有零码间干扰的数字PAM系统的差错概率

## 6.5 部分响应系统——具有受控码间干扰的带限系统

上节说到，采用升余弦频谱信号可以实现零码间干扰，但是这样会降低符号率和频谱利用率，所以可以通过采用部分相应技术，人为引入可控的码间干扰来，同时达到奈奎斯特码率$T =1/(2W)$

### 一、双二元信号脉冲

设计发送和接收滤波器使得复合信号满足
$$
x(nT)=\begin{cases}1, &n=0,1\\ 0, &else \end{cases}
$$
则类似于奈奎斯特定理，其等价于
$$
\sum_{m=-\infty}^{\infty}X(f+\frac{m}{T}) = T + Te^{-j2\pi fT},\ 0<f<\frac{1}{T}
$$
这种情况下，$T =1/(2W)$时有
$$
X(f)=\begin{cases}\frac{1}{2W}[1+e^{-j\frac{\pi f}{W}}], & |f|<W\\ 0, & else \end{cases}\\
x(t) = simc(2Wt)+sinc(2W(t-1))
$$
此时的$x(t)$称为双二元信号脉冲。频谱图如下：

<img src="双二元信号.png" alt="双二元信号" style="zoom:50%;" />

### 二、带有受控码间干扰数据的检测

若采用上一节所说的双二元信号作为基本脉冲波形，则发送和接收滤波器应该满足：
$$
|G_T(f)| = |G_R(f)| = |X(f)|^{\frac{1}{2}}
$$
假设发送序列$a\{n\},a\{n\} \in \{-1,1\}$，则接受滤波器输出$t = mT$的采样值为：
$$
y_m=y(mT)=a_m+a_{m-1}+\xi_m
$$

> 可以看出前一时刻的传输的数据严重影响了这一时刻的数据输出。可以采用两种方法消除这种码间干扰

1. 在接收端做减法，用m时刻接收到的值减去m-1时刻的值。但是这种方法会造成错误的传递，一旦有一个数据出错，后面会连续错很多位。
2. 在发送端预编码。这是广泛采用的方法，所以后文详细介绍这一方法。

### 三、采用部分响应信号的数字PAM差错概率
...
前面讨论时候，我们是假设$C(f)$为带宽为W的理想举行，相位谱为线性的。从而通过设计使得发送滤波器、信道、接受滤波器组合起来满足升余弦或者双二元信号频谱，达到无码间干扰的设计。
$$
G_T(f)\cdot C(f)\cdot G_R(f) = X_{rc}(f)
$$
这一节我们讨论当$C(f)$存在幅度失真或者相位失真情况下的系统设计。

### 一、信道特性已知，发送和接收滤波器的设计

为了消除码间干扰，发送接收滤波器和信道满足：
$$
G_T(f)\cdot C(f)\cdot G_R(f) = X_{rc}(f)e^{-j\pi f t_0}
$$
当$C(f)$已知时，接收与发送滤波器的设计就变得简单了起来：
$$
G_T(f)=\frac{\sqrt{X_{rc}(f)}}{C(f)}e^{-j2\pi ft_0}
$$


$t_0$是因为因果性引入的延时，在这种情况下，接收滤波器接收到的信号就为：
$$
G_T(f)C(f)=\sqrt{X_{rc}(f)}e^{-j2\pi ft_0} 
$$


而前面章节的讨论已经告诉了我们，在高斯白噪声情况下，接收滤波器设计成与被接收信号相匹配的形式能够得到最大信噪比，所以接收滤波器被设计为：
$$
G_R(f)=\sqrt{X_{rc}(f)}e^{-j2\pi t_r}
$$


### 二、信道特性未知，发送和接受滤波器的设计——信道均衡器
