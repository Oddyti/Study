...
R = 0.5; % 滚降系数
span = 40; % 滤波器包含符号数
sps = T_last; % 每个符号所含采样点数
Hd = rcosdesign(R, Nsym, 2, 'sqrt'); % 滤波器设计
modSigI = upfirdn(seq_I, Hd, Nss); % 上采样
modSigQ = upfirdn(seq_Q, Hd, Nss); % 上采样
...
V_pi = 3.5; % 半波电压
V_bias = 3.5; % 偏置电压
[E_out_I, ut_I] = MZM(Et_1, modSigI, V_pi, V_bias, T_last);
[E_out_Q, ut_Q] = MZM(Et_1, modSigQ, V_pi, V_bias, T_last);
drawfft(real(E_out_I), fs, 1, 'Eout_I');
drawfft(real(ut_I), fs, 2, 'ut_I');

%% UTC-PD产生太赫兹波
% 这里看频谱图有问题，理论上需要提高码元宽度
R = 0.5; % UTC_PD的响应度
E_thz_I = UTC_PD(E1, E2, fc_1, fc_2, ut_I, ut_Q, R, Ts);
