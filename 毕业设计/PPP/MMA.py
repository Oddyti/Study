...
class MMA():
    ...
    def mma(self,u_in,E_0,V_bias,V_pi,landa):
        ...
        for i in range(self.K):
            ...
            delta_J=(y_r*(math.pow(y_r,2)-Rr)+1j*(y_i*(math.pow(y_i,2)-Ri)))*conjU
            
            w[:,i+1]=w[:,i]-landa*delta_J #权重更新

            y_t.append(np.sum(w[:,i+1]*U))
        
        return [w[:,self.K].reshape(1,self.N),y_t]
