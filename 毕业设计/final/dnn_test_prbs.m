...
err_ratio_all = zeros(5,11); % 记录每种模式不同SNR的误码率
for SNR = 0:10
    fprintf('SNR = %d\n',SNR);
    % 信号传输
    ...
    for nn_mod = 1:5
        ...
        switch nn_mod
            ...
            %     x_test  = x_test_mix;
            %     y_test  = y_test_mix;
            case 2  % Train: prbs, Test: mt
                x_train = x_train_prbs;
                y_train = y_train_prbs;
                x_test  = x_test_mt;
                y_test  = y_test_mt;
            case 3  % Train: prbs, Test: prbs
                x_train = x_train_prbs;
                y_train = y_train_prbs;
                x_test  = x_test_prbs;
                y_test  = y_test_prbs;
        ...
        % NN网络设置
        hidden_n = mem_n - 5;
        net = newff(real(x_train), real(y_train), [hidden_n, hidden_n] , {'tansig' 'tansig'} , 'traingdx' );
...
plot(x, err_ratio_all(3,:),'-*','color',color2/255,'linewidth',2);hold on;
plot(x, err_ratio_all(4,:),'-x','color',color3/255,'linewidth',2);hold on;
plot(x, err_ratio_all(5,:),'-s','color',color3/255,'linewidth',2);hold on;
...
legend('mt-mt','prbs-mt','prbs-prbs','mix-mt','mix-mix','Location','Northeast')
...
semilogy(x, err_ratio_db(3,:),'-*','color',color2/255,'linewidth',2);hold on;
semilogy(x, err_ratio_db(4,:),'-x','color',color3/255,'linewidth',2);hold on;
semilogy(x, err_ratio_db(5,:),'-s','color',color3/255,'linewidth',2);hold on;
