...
for i = 1:16
    ...
    for nn_mod = 1:11
        ...
        % NN网络设置
        hidden_n = mem_n + 2;
%         net = newff(real(x_train), real(y_train), [hidden_n, hidden_n], {'tansig' 'tansig'}, 'trainrp');
        net = fitnet(26);
...
figure(2);
semilogy(x, err_ratio_db(1, :), '-o', 'color', color1 / 255, 'linewidth', 2.5); hold on;
semilogy(x, err_ratio_db(2, :), '-+', 'color', color2 / 255, 'linewidth', 2.5); hold on;
legend('MT序列对照组', '不同PRBS序列', 'Location', 'Southeast')
xlabel('PRBS阶数');
ylabel('BER(dB)');
