...
def MnistConv(W1, W5, Wo, X, D):
    ...
    for k in range(N):
        ...
        # one-hot encoding
        d = np.zeros((10, 1))
        d[D[k][0]][0] = 1 #将其更改为10*1的符号为0，1的向量
        
        # Backpropagation
        e     = d - y
        delta = e

        e5     = np.dot(Wo.T, delta)    # Hidden(ReLU)
        delta5 = (y5 > 0) * e5
        
        e4 = np.dot(W5.T, delta5)       # Pooling layer
        ...
        dW1 =delta1_x
        dW5 =np.dot(delta5, y4.T)
        dWo =np.dot(delta, y5.T)
        ...
        #     dW1 = dW1 + delta1_x
        #     dW5 = dW5 + np.dot(delta5, y4.T)
        #     dWo = dWo + np.dot(delta, y5.T)
