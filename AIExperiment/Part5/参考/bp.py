...
class BP:
    def __init__(self , layer , iter , maxError):
        #初始化输入、隐藏、输出层的节点数目
        self.inputN = layer[0]
        self.hideN = layer[1]
    ...
    def forwardBackwordMomentum(self , data , dest , learnRate = 0.5 , beta = 0.9):
    ...
    def forwardBackwordSGD(self , data , dest , learnRate = 0.5):
        assert isinstance(data , list) , "输入data不为list"
        assert isinstance(dest , list) , "输入dest不为list"
        data = np.array(data)
        dest = np.array(dest)
    ...
    def fitSGD(self , X , y):
        for i in range(self.iter):
            ...
            for j in range(len(X)):
                error += abs(self.forwardBackwordSGD(X[j] , y[j]))
            error = error.sum()
    ...
    def fitMomentum(self , X , y):
        for i in range(self.iter):
            error = 0.0
            for j in range(len(X)):
                error += abs(self.forwardBackwordMomentum(X[j] , y[j]))
            error = error.sum()
